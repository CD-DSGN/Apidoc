...
- 2000010          未查找到要搜索的相关图书信息
- 2000011          未查询到热门关键词的结果
- 2000012          数据库没有该书记录
- 2000013          关注图书成功,添加扫描记录失败
- 2000014          该用户已关注过此书,添加扫描记录失败
- 2000015          数据库无此书籍,记录失败
- 2000016          图书评分数字格式错误
- 2000017          图书评分失败
...
| ------------ | --------- | --------- | ----------- | ---- | ---- |
| access-token | 用户授权token | 用户授权token | string | 是    | 无    |
| count        | 页面显示的条数| 每页显示内容的条数 | integer     | 否    | 8    |
...
| 参数名          | 含义        | 规则说明        | 参数类型        | 是否必须 | 缺省值  |
| ------------ | --------- | ----------- | ----------- | ---- | ---- |
|user_id2| 好友ID    | 好友表中用户对应的好友ID | integer(11) | 是    | 无    |
